...
handler.post(async (req, res) => {
  ...
  try {
    ...
    if (teacher != "" && picture != "") {
      ...
      const newRegister = await Subject.create({
        subjectname: subjectname,
        adminId: adminId,
        classId: classId,
        picture: picture,
        teacher: teacherfind._id,
      });
      newRegister.save();
      ...
      const enrollTeacher = await Teacher.findOneAndUpdate(
        { firstname: teacher, adminId: adminId },
        { $push: { subject: newRegister._id } }
      );
