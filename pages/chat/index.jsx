...
function Index({ session, joke }) {
  ...
  // set the users online via the socket
  useEffect(() => {
    ...
    if (session) {
      try {
        const response = await axios.get(
          `/api/${session?.user?.section}/${session?.user?._id}`
        );
  ...
  //enable the conversation, creates a conversation
  ...
  const onSearch = async (value) => {
    ...
    if (value != "" && res != session?.user?.email) {
      ...
      try {
        ...
        if (response) {
          // create a conversation between the user and you
  ...
  return (
    <>
      {session ? (
        <div
          style={{ height: "100vh" }}
          className="bg-gray-900  overflow-x-hidden overflow-y-scroll"
        >
