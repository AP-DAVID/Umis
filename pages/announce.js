...
function Index({ session, joke }) {
  ...
  // get all scripts/blogs using the userId, if it is the adminId logged in
  useEffect(async () => {
    if (session) {
      try {
        if (session?.user?.section === "admin") {
          const response = await axios.get(
            `/api/getScript/${session?.user?._id}`
          );
      ...
      {session.user && showModal === false ? (
        <div className="h-screen overflow-y-scroll scrollable bg-gray-700">
          ...
          <div className="sm:px-7  ml-16 sm:ml-1 mr-4 sm:mr-0 justify-items-stretch sm:space-x-1 sm:grid md:grid:cols-2 lg:grid-cols-2 xl:grid-cols-3 3xl:flex flex-wrap justify-center">
           {/* card to input blog/script details */}
            {data?.map((content) => (
              <Card
                key={content._id}
          ...
          >
            <div className="invisible group-hover:visible  ">
              <h1 className="text-sm text-white font-md">add</h1>
            </div>
            <div className="rounded-full px-3 py-3 bg-blue-500">
              <PlusIcon className="hover:animate-spin h-10 w-10" />
            </div>
          </div>
        </div>
