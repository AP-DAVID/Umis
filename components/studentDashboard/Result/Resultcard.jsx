...
function Resultcard({data, result}) {
    ...
    return (
        <div>
            ...
            >
                <div className="h-1/3 bg-no-repeat justify-between flex flex-col bg-cover bg-gray-600 w-full rounded-t-lg " style={{ backgroundImage: `${ data?.picture ? `url("${data?.picture}")` : 'url("https://images.unsplash.com/photo-1506220926022-cc5c12acdb35?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Nnx8YmxhY2slMjBiYWNrZ3JvdW5kfGVufDB8fDB8fA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60")'}`}}>
                    <div className="flex justify-between ml-3">
                        <h1 className="text-white text-xl font-semibold">{result.classname} </h1>
                ...
                <div className="flex justify-between ml-3 mr-2">
                        ...
                        <div className="bg-gray-300 object-cover items-center h-12 w-12 filter rounded-full lg:rounded-3xl brightness-75 justify-self-end ">
                            ...
                            <UserIcon
                                className="text-blue-400 align-middle ml-1 items-center w-10  group-hover:animate-bounce"
                            />
                        </div> 
            ...
            {
                openModal && (
                    <List setOpenModal={setOpenModal} d={result} result= {result.results}/>
                )
            }
        </div>
    )
