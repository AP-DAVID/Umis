...
function Modal({ setShowModal, showModal, login }) {
  ...
  const sendEmail = async () => {
    ...
    await setToSend(
      (toSend.message = `Your username is : ${form.username} and your password is : ${form.password}, you can log in to your student's panel with the credentials`)
    );
  ...
  const handleSubmit = async () => {
    if (
      ...
      try {
        ...
        try {
          const response = await axios.post(`/api/admin/student`, form, config);
        ...
        <div className="relative w-full sm:w-2/3 my-auto mx-auto max-w-full">
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
              <h3 className="text-3xl font-semibold">Create Student</h3>
            </div>
            {/*body*/}
            ...
            <form className="h-full w-full space-y-2 ml-6 sm:px-5 my-10 sm:mr:10 justify-items-stretch sm:space-x-1 sm:grid md:grid:cols-2 lg:grid-cols-2 xl:grid-cols-3 3xl:flex flex-wrap justify-center">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  name="contact"
                  className="h-16 px-3 text-indigo-500 font-bold  bg-gray-200 shadow-md  w-54 rounded-full "
                />
              </div>
