  ...
  return (
    <div className="flex px-2 space-y-4 flex-col items-center py-5">
      ...
      <div
        ...
        className="px-5 py-5 h-44 sm:h-56 w-full sm:w-3/4 flex flex-col justify-end bg-gray-700 rounded-lg shadow-md "
      >
        <div className="flex flex-col space-y-2 text-white">
          <h1 className="text-4xl text-white font-medium">
            {" "}
            {data?.subject[0]?.subjectname}{" "}
          </h1>
          <h1 className="text-2xl text-white">
            {" "}
            {data?.teacher[0]?.firstname} {data?.teacher[0]?.lastname}
          </h1>
        </div>
      </div>
      ...
      <div className="w-full flex-col space-y-5 sm:w-3/4">
        {!lock && (
          <div
            ...
            className="flex px-4 py-4 space-x-4 cursor-pointer items-center shadow-lg border-t-2 rounded-lg group"
          >
            <div>
              <UserCircleIcon className="h-10 w-10 text-purple-500" />
            </div>
            <div className="font-light group-hover:font-medium group-hover:text-purple-500">
              <h1>Announce Something to the class</h1>
            </div>
          </div>
        )}
        ...
        {lock && (
          <div>
            ...
            <div className="flex py-2 px-2 justify-between items-center">
              <XIcon
                onClick={() => setLock(!lock)}
                className="h-9 hover:text-red-500 w-8 text-blue-500"
              />

              <ChevronDoubleRightIcon
                onClick={handleSubmit}
                className="h-10 w-10 hover:text-red-500 text-blue-500"
              />
            </div>
          </div>
        )}
        ...
        {data.text.length === 0 ? (
          <div className="px-4 border rounded-lg py-4 ">
            <div className="text-2xl">
              {" "}
              View class updates and connect with class here
            </div>
