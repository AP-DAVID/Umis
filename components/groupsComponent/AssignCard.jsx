...
function AssignCard({ title, body, details, data }) {
  ...
  var found = details.submission.findIndex(function (post, index) {
    if (post.student[0]._id === data?._id) return true;
  });
  ...
  return (
    ...
    >
      <div
        onClick={
          data.section === "student" && found === -1
            ? () => setOpenModal(true)
            : () => setOpen(true)
        }
        className="flex space-x-4 ml-3 break-all"
      >
      ...
      {openModal && details.close != true && (
        <AnswerModal
          body={body}
          title={title}
          assignId={details?._id}
          studentId={data?._id}
      ...
      {openModal &&
        details.close === true &&
        message.error("Submission has been closed")}
