...
function Submissions({setOpenn, data, login, index}) {
  ...
  const handleSubmit = async () => {
      ...
      try{
        ...
        const response = await axios.put(`/api/assign/input/${data?._id}`, form, config);
   ...
   //login is for the user
    ...
    return (
        <div>
             <div
          className="justify-center h-screen px-2  py-2 w-screen items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none"
        >
          <div className="relative w-full sm:w-2/3 my-auto mx-auto max-w-full">
            ...
            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
              ...
              <div className="flex justify-center">
                    <h1 className="font-medium text-red-500 text-center p-4 text-base">{messagee}</h1>
              </div>
            ...
            <div className = "px-5">
                {
                  index === -1 ? (
                    ...
                    data.submission.map((s) => (
                      <SubmitCard details={data} key={s._id} data={s} index = {index} date={s.createdAt} answer={s.answer} mark={s.mark} student={s.student}/>
                   ))

                  ): (
                    
            
                         <SubmitCard details={data} key={submit._id} data={submit} index={index} date={submit.createdAt} answer={submit.answer} mark={submit.mark} student={submit.student}/>
...
{/*                      
              ...
              <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
               ...
               <button
                ...
                {
                    index === -1 && data?.close != true && login?.section === "teacher" ? (
                      <button
