...
function Comment({open, setOpen, comment, image, login, data, }) {
    ...
    useEffect( () => {
      setTimeout(async()=>{ await scrollRef?.current?.scrollIntoView({ behavior: "smooth" }); }, 500);
  ...
  return (
    ...
    >
           ...
           <div  className="md:w-3/4 ">
               {
                   comment.map((c) => (
                       ...
                       <Cardc key={c?._id} data={c} section = {c?.sectionId} login={login} text={c?.text} picture = {c.picture} name = {`${c?.firstname} - ${c?.lastname}`}/>
      ...
      <Modal.Actions  className="flex  justify-evenly">
        ...
        <textarea ref={scrollRef} type="text" name = "text" value={form.text} onChange={handleChange}  placeholder="add a comment" className="w-full px-2 py-2"/>
